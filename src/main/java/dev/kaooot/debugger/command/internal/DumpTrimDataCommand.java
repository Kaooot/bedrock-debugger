 ...
 */
@Name("dumptrims")
@Description("Outputs valid trim data nbt in a binary format.")
@Overloads({
...
public class DumpTrimDataCommand extends DumpPaletteCommand {

    @Override
    public void execute(String command, String[] args, BedrockDebuggerProxy proxy) {
        this.execute0(args, "trim_data", proxy.getPlayer().getTrimData(), proxy);
    }
}